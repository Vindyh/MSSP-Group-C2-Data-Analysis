...
```{r}
df <- read_delim("AirQualityData.csv", 
    delim = ";", escape_double = FALSE, trim_ws = TRUE)

summary(df)
```
```{r}
colnames(df)[3] <- "GroundCO"
df
```
```{r}
#replace all missing values with median
#get boxplots for each of the variables and the best case if you get side by side box plots
med <- median(df$GroundCO, na.rm = FALSE)
med
is.na(df)
```
